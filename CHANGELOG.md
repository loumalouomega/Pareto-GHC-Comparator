...
- Claude Code usage history, as a separate opt-in source. The Usage tab can now read Claude Code's own local transcripts alongside your Copilot chat sessions, and shows them in their own tables: turns, input, output, cache-read, and cache-write tokens per model, per day, and per workspace. It is a genuinely separate ledger, not an extra column: Copilot counts premium requests or AI credits while Claude Code counts tokens under a different billing model, so the two are never added together, no price is shown for either, and the budget suggestion and the "only my models" filter still speak only about Copilot. Claude Code's own cost figure is not treated as a rate, and model names are shown exactly as the client reports them rather than matched to a priced registry entry by guesswork. Nothing is read until you include Claude Code through its own confirmation, which states that token counts, the model name, timestamps, and workspace paths are read but message content is not — Claude Code's memory files in the same folder are never opened — and that `claude project purge` is the client's own way to delete them. Upgrading never adds the source on your behalf, and **Stop and remove** for Claude Code deletes only its stored data, leaving every other source untouched. Subagent turns are counted separately and held out of the totals rather than double-counting the work they inherit, and a turn whose token counts cannot be read is reported as such instead of being shown as zero. Because Claude Code does not publish its transcript format, a transcript that no longer matches is reported with a content-free fingerprint naming the client version and which fields changed, so a format change is a reportable observation rather than a silent zero. The status bar and the erase command now read "usage" rather than "Copilot usage", because watching can cover both clients at once.
- Weekly upstream schema-drift reporting for the OpenCode CLI (maintainers). A scheduled, non-gating check runs the real credential-free discovery smoke test against both the pinned OpenCode release and the latest one, and files or updates a single tracked issue when — and only when — the pinned release lists models and the latest one does not. Failures an outage explains (a download failure, a timeout, an unresolvable binary, or simply no models connected) are recorded on the run and deliberately file nothing, so the lane is not a stream of false alarms, and a healthy week closes the tracked issue instead of leaving it open. Each report names the failure kind, the detected CLI version, and content-free counters — field-presence counts only, never model names, rates, or configuration. The check runs in its own workflow so it can never gate a release, and the single step permitted to write issues never runs the downloaded program; it only reads the sanitized results. OpenCode's own documentation still advertises a flag its current release rejects, so a release that changes this output is now noticed in days rather than by the first user who hits it. Copilot Chat is unchanged and still has no equivalent check: it has no command-line surface to run, so its only signal remains the schema fingerprint a user reports.
...
- Chart accessibility pass: the comparison charts are now usable without a mouse or without colour perception. Model colours come from a curated colour-blind-safe palette assigned stably per model (two models in one view never share a colour), and a row's status is no longer carried by colour alone — a ring marks the Pareto frontier, a star marks a recommendation, and the size marks local usage, all documented in the chart's own caption. Focusing a chart and pressing the arrow keys walks its plotted models, Home/End jump to the ends, and Enter selects the focused one, with each step announced; the results table repeats the chart's quadrant and dominator facts, and a screen-reader description of the chart carries the same information sentence by sentence. Quadrant membership is only shown while the shaded region is drawn, so the table and the chart can never disagree.
- Additional Copilot usage sources: local usage scanning is no longer limited to VS Code and VS Code Insiders. The Usage tab now lists every known chat-session storage root found on this machine — the VS Code forks that keep the same layout (VSCodium, Cursor, Windsurf, Code - OSS, Trae) and the VS Code Server roots used by Remote-SSH, WSL, and dev container sessions — and each one is added only through its own consent prompt, stating exactly what it reads. Finding an editor never reads it: existence is checked without opening anything inside a root you have not included, and an editor that turns out to hold no Copilot sessions reports that instead of inventing totals. Upgrading never grants a new source — an existing consent keeps covering exactly the two VS Code roots. Included editors are listed with their own **Stop and remove**, which deletes just that editor's stored data and leaves the rest untouched, and the Usage tab now splits totals per editor, naming the source editor on workspace rows once more than one contributes.
- Read-only snapshot import: **Import snapshot JSON** (Export & import card, or the `Pareto GHC: Import Snapshot JSON (Read-Only)` command) reopens a snapshot JSON you exported earlier as historical data — a persistent banner names the file, its export date, the cost basis and unit, and the registry dates as of that export, and says in plain words that it is not live results. Costs stay on the snapshot's own basis (the workload inputs and Chart view are locked, so exported numbers are never relabelled or repriced), the recommendation is the one recorded at export rather than a recomputation, and rows show only what the export contains, with model details reduced to read-only mapping and pricing provenance. The text filter, chart display toggles, and the custom comparison tray still work on the exported rows; mapping, pricing, pins, exclusions, workload profiles, and exports are unavailable, and any attempt to change them is refused with an explanation instead of altering your saved settings. Single-option and two-option snapshots both open (a pair reopens as two read-only panels with its B-minus-A delta). A file that is not a snapshot, an older or newer schema version, or a corrupted one fails with a message naming the reason and leaves the live comparison untouched; only the picked file's path is remembered, so reopening the panel shows the same snapshot until you return to live data.
- Watchlist change alerts (opt-in, off by default): a Settings toggle under Included models sends one VS Code notification per benchmark refresh when a pinned model's price, benchmark score, mapping status, or availability changed since the previous snapshot, citing both snapshot versions and retrieval dates with an Open-comparison action. Sub-threshold score drift, unknown scores, excluded pins, and pins from other sources never alert.
- Workload sensitivity view: a **Workload sensitivity** card sweeps the output share of the token workload (0–100% at a fixed total) and compresses the result into ranges where the recommendation and Pareto frontier stay constant, so the ranking's dependence on the workload assumption is visible at a glance. Labelled throughout as a what-if — illustrative, never measured cost or usage history; an empty workload falls back to a stated illustrative total, and legacy billing yields one flat range.
