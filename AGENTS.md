...
`src/opencode.ts` exposes `executableCandidates`, `createOpenCodeRunner`, and `execOnce` with injectable environment/platform/process boundaries. Native executable paths are passed directly to `execFile`, without a shell; Windows uses `.exe` and Windows path semantics. The home fallback follows PATH candidates. Timeout is a distinct failure category. `test/opencode-platform.test.ts` covers actual native invocation at a path with spaces plus missing binaries and timeout limits; the CI platform-discovery matrix runs on Linux, macOS, and Windows and gates release jobs alongside the existing build. Local verification does not establish remote matrix success or actual provider discovery.
