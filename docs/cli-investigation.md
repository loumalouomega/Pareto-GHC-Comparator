...
- Running it against `test/fixtures/cli/{snapshot,options}.json` with `--source codex` prints a 6-row CSV (one row per known Codex model) with correct pricing/mapping status, and with `--format snapshot` prints a version-1 JSON snapshot — both byte-identical in shape to what `src/extension.ts` writes today.
