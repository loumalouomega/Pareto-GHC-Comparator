...
How AI-credit, legacy premium-request, and OpenCode USD costs are estimated for an illustrative token workload. Cost units never mix: Copilot models compare in AI credits or legacy billing, OpenCode models in USD. There is no cross-unit frontier; comparison mode can optionally show a USD equivalent alongside the native cost (see below). The comparison overlay view is the one labelled exception: it plots both options on a shared, explicitly labelled axis — native units when they match, a USD equivalent when they don't (see "Overlay view" below).
...
Compare options has a **View** control next to **Show USD equivalents**: **Side by side** (default) keeps the original two-chart layout; **Overlay** superimposes both options' models on one chart, so the better-value tool is visible at a glance instead of read off two separate axes.

- **Axis unit**: native cost when both options bill the same way (e.g. both AI credits, or both legacy on the same plan). Otherwise every plotted row is converted to its USD equivalent using the same rate and rules as **Show USD equivalents** above, and the axis is labelled "USD equivalent". This is the one place a chart's x axis mixes what would otherwise be different native units — it's always an explicit, labelled equivalent, never blended silently.
- **What's excluded**: legacy premium requests never convert (same reason as above), so when one option is legacy and the other isn't, that option's rows are dropped from the overlay rather than plotted on a unit they don't belong to; a notice states how many rows and from which side.
- **Frontiers**: each option keeps its own Pareto frontier line. A third, combined frontier is also drawn across both options' rows together — using the same dominance rule (equal-or-better cost and score, with one strict improvement) — so you can see which tool actually wins at each cost level. The combined frontier only appears when the two options are otherwise comparable: same cost basis, same workload (workload view only), same legacy plan when both are legacy. When they aren't, a notice explains why and only the per-option frontiers are shown — converting units doesn't make different workloads comparable.
- **Scope**: the overlay is a display-only merge of already-computed rows; it never changes either option's own chart, frontier, or recommendation, and it isn't exported separately (PNG export draws the overlay canvas when that view is active, in place of the two per-option charts).
